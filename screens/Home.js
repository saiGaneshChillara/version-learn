  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        placeholder="Search by subject, topic, or file name"
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {Object.keys(filteredSubjects).length === 0 ? (
          <Text style={styles.noDataText}>No matching subjects, topics, or files found.</Text>
        ) : (
          Object.keys(filteredSubjects).map(subject => (
            <View key={subject} style={styles.subjectCard}>
              <Text style={styles.subjectTitle}>{subject}</Text>
              {filteredSubjects[subject].map(item => (
                <TouchableOpacity
                  key={item.id}
